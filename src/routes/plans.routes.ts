 ...
 * CACHED: 1 hour (public data)
 */
router.get('/studio/boosters',
  cacheMiddleware.cache({
    namespace: CacheNamespace.PUBLIC,
    ttl: CacheTTL.LONG, // 1 hour
    keyGenerator: (req) => {
      const region = (req as any).region || Region.INDIA;
      return `plans:boosters:all:${region}`;
    }
  }),
  (req: Request, res: Response) => {
    try {
 ...
 * CACHED: 1 hour (public data)
 */
router.get('/studio/boosters/:boosterId',
  cacheMiddleware.cache({
    namespace: CacheNamespace.PUBLIC,
    ttl: CacheTTL.LONG, // 1 hour
    keyGenerator: (req) => {
      const region = (req as any).region || Region.INDIA;
      const boosterId = req.params.boosterId || 'UNKNOWN';
      return `plans:boosters:single:${boosterId}:${region}`;
    }
  }),
  (req: Request, res: Response) => {
    try {
