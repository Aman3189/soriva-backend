...
CREATE TABLE IF NOT EXISTS orbit_keywords (
  ...
  CONSTRAINT unique_user_orbit_keyword UNIQUE (user_id, orbit_id, keyword)
);

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_orbit_suggestions_user ON orbit_suggestions(user_id);
CREATE INDEX IF NOT EXISTS idx_orbit_suggestions_conversation ON orbit_suggestions(conversation_id);
CREATE INDEX IF NOT EXISTS idx_orbit_suggestions_choice ON orbit_suggestions(user_choice);
CREATE INDEX IF NOT EXISTS idx_orbit_keywords_user_orbit ON orbit_keywords(user_id, orbit_id);
CREATE INDEX IF NOT EXISTS idx_orbit_keywords_keyword ON orbit_keywords(keyword);